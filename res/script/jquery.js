...
levels = localStorage.getItem("animeLevels");
total = parseInt(levelTotal);

var coins = localStorage.getItem("aCoins");
$('#coins').text(coins);
...
function Retryminus() {
	if (coins >= 1) {
		...
		document.getElementById("aCoins").innerHTML = coins;
	}
}
...
$(document).ready(function () {
	...
	$('.btn1').click(function () {
		setInterval(() => {
			coins = localStorage.getItem("aCoins");
			$("#coins").html(coins);
		}, 2000);
...
function AppenAll() {
	...
	for (ws in splittedAns) {
		...
		if (splittedAns[ws].length < 7 && singleLine < 10) {
